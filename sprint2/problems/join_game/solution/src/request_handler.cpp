...
namespace http_handler {
    ...
    std::string RequestHandler::MapsListResponse() const{
    ...
    StringResponse RequestHandler::ReportServerError(unsigned version, bool keep_alive) const{
        return MakeStringResponse(http::status::not_found, "Server Error."s, version, keep_alive, ContentType::TEXT_PLAIN);
    ...
    std::string RequestHandler::DecodeUri(const std::string& uri_str) const{
        std::string result;
    ...
    char RequestHandler::DecodeChar(std::istream& in) const{
